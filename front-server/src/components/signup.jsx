...
function SignUp({ onSwitchToLogin, isActive }) {
	...
	const sendVerificationEmail = async () => {
		...
		try {
			const response = await fetch('http://localhost:7100/api/email/send-verification', {
				method: 'POST',
				headers: {
	...
	const verifyCode = async () => {
		...
		try {
			const response = await fetch('http://localhost:7100/api/email/verify-code', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
					...
					{step === 1 ? (
						<div className="signup__forms">
							...
							<div className={getClassName('field', 1)}>
								...
								>
									{emailVerification.isSending ? '전송 중...' : '이메일인증'}
								</button>
							</div>
							{emailVerification.showVerificationInput && !emailVerification.isVerified && (
								<div className={getClassName('field', 1.5)}>
									<input 
										className="text" 
										type="text" 
