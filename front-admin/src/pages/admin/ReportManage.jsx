...
import { fetchReports, resolveReport } from '../../services/api'; // 실제 API 함수 import
...
function ReportManage() {
  ...
  useEffect(() => {
    ...
    fetchReports({ page: 0, size: 100 })
      .then(res => setReports(res.data)) // res.data는 백엔드에서 반환하는 신고 배열
  ...
  const handleResolve = async (reportId) => {
    try {
      ...
      setReports(prev =>
        prev.map(report =>
          report.reportId === reportId ? { ...report, status: 'RESOLVED' } : report
        )
      );
      alert('신고가 처리되었습니다.');
    } catch (err) {
      ...
      <ul>
        {reports.map(report => (
          <li key={report.reportId}>
            {report.reporterName} → {report.reportedUserName} : {report.reason} [{report.status}]
            {report.status !== 'RESOLVED' && (
              <button onClick={() => handleResolve(report.reportId)}>처리</button>
            )}
          </li>
        ))}
      </ul>
