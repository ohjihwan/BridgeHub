...
function Statistics() {
  ...
  return (
    <div>
      <h2>관리자 통계</h2>
      <div>
        <strong>전체 회원 수:</strong> {stats.totalMembers}명<br />
        <strong>활성 회원 수:</strong> {stats.activeMembers}명<br />
        <strong>정지 회원 수:</strong> {stats.bannedMembers}명<br />
        <strong>신고 건수:</strong> {stats.reportCount}건<br />
      </div>
      <div style={{ marginTop: 20 }}>
        <h3>성별 통계</h3>
        남성: {stats.genderStats.male}명 / 여성: {stats.genderStats.female}명
      </div>
      <div style={{ marginTop: 20 }}>
        <h3>연령대 통계</h3>
        <ul>
          {stats.ageStats.map(item => (
            <li key={item.range}>{item.range}: {item.count}명</li>
          ))}
        </ul>
      </div>
      <div style={{ marginTop: 20 }}>
        ...
        <ul>
          {stats.recentReports.map(report => (
            <li key={report.id}>
              {report.date} - {report.reporter} → {report.target} ({report.reason})
            </li>
          ))}
        </ul>
      </div>
