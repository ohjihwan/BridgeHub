...
export default function Statistics() {
  ...
  const [apiChartData, setApiChartData] = useState(chartDataList);
  const [barData, setBarData] = useState(barData);
  const [topActiveUsers, setTopActiveUsers] = useState(topActiveUsers);
  const [recentReports, setRecentReports] = useState(recentReports);
  const [popularRooms, setPopularRooms] = useState(popularRooms);
  const [totalVisitors, setTotalVisitors] = useState(5000000);

  // API에서 통계 데이터를 불러오는 새로운 함수
  const loadStatistics = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // 회원 통계를 차트 데이터로 변환하는 새로운 로직
        if (data.memberStats) {
          const updatedChartData = apiChartData.map(chart => {
            switch (chart.id) {
              case 'gender':
                  ...
                  return {
                    ...
                    data: {
                      ...
                      datasets: [{ 
                        ...
                        data: [data.memberStats.gender['남성'] || 0, data.memberStats.gender['여성'] || 0]
                      }]
                    }
                  };
                }
                  ...
                  return {
                    ...
                    data: {
                      ...
                      datasets: [{ 
                        ...chart.data.datasets[0],
                        data: [
                          data.memberStats.education['고졸'] || 0,
                          data.memberStats.education['대학교'] || 0,
                          data.memberStats.education['대학원'] || 0
                        ]
                      }]
                    }
                  };
                  ...
                  return {
                    ...
                    data: {
                      ...
                      datasets: [{ 
                        ...chart.data.datasets[0],
                        data: [
                          data.memberStats.time['06:00~12:00'] || 0,
                          data.memberStats.time['12:00~18:00'] || 0,
                          data.memberStats.time['18:00~24:00'] || 0
                        ]
                      }]
                    }
                  };
                  ...
                  return {
                    ...
                    data: {
                      ...
                      datasets: [{ 
                        ...chart.data.datasets[0],
                        data: [
                          data.memberStats.major['인문•사회'] || 0,
                          data.memberStats.major['상경'] || 0,
                          data.memberStats.major['자연'] || 0,
                          data.memberStats.major['공학'] || 0,
                          data.memberStats.major['예체능'] || 0,
                          data.memberStats.major['의학'] || 0,
                          data.memberStats.major['법학'] || 0,
                          data.memberStats.major['융합'] || 0
                        ]
                      }]
                    }
                  };
          ...
          setApiChartData(updatedChartData);
          setSelectedChart(updatedChartData[0]);
        }

        // 활동 통계 처리하는 새로운 로직
        if (data.activityStats) {
          // 분기별 데이터 처리
          if (data.activityStats.quarterlySignups && data.activityStats.quarterlyVisitors) {
            setBarData({
              labels: ['q1', 'q2', 'q3', 'q4'],
              datasets: [
                {
                  label: '신규 가입자 수',
                  data: [
                    data.activityStats.quarterlySignups.q1 || 0,
                    data.activityStats.quarterlySignups.q2 || 0,
                    data.activityStats.quarterlySignups.q3 || 0,
                    data.activityStats.quarterlySignups.q4 || 0
                  ],
                  backgroundColor: '#6a6cff'
                },
                {
                  label: '총 접속자 수',
                  data: [
                    data.activityStats.quarterlyVisitors.q1 || 0,
                    data.activityStats.quarterlyVisitors.q2 || 0,
                    data.activityStats.quarterlyVisitors.q3 || 0,
                    data.activityStats.quarterlyVisitors.q4 || 0
                  ],
                  backgroundColor: '#e573b7'
                }
              ]
          ...
          // TOP 활동 사용자 처리하는 새로운 로직
          if (data.activityStats.topActiveUsers) {
            setTopActiveUsers(data.activityStats.topActiveUsers);
          }

          // 인기 채팅방 처리하는 새로운 로직
          if (data.activityStats.popularRooms) {
            setPopularRooms(data.activityStats.popularRooms);
          }

          // 총 방문자 수 처리하는 새로운 로직
          if (data.activityStats.totalVisitors) {
            setTotalVisitors(data.activityStats.totalVisitors);
          }
        }

        // 신고 통계 처리하는 새로운 로직
        if (data.reportStats && data.reportStats.recentReports) {
          setRecentReports(data.reportStats.recentReports);
        }
      }
    } catch (error) {
      console.error('통계 데이터 로딩 실패:', error);
      // API 실패 시 기본 더미 데이터 유지 (새로운 에러 처리 로직)
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="dashboard-admin statistics-final">
      {/* TODO: 실제 API 연동 시 구현 예정 */}
      <div style={{ textAlign: 'center', padding: '40px' }}>
        <h2>통계 페이지</h2>
        <p>API 연동 후 구현 예정입니다.</p>
      </div>
      
      {/* TODO: 실제 API 연동 시 주석 해제 예정
      ...
      <div className="statistics-main-grid-final" style={{ display: 'flex', flexWrap: 'wrap', gap: 32, margin: '40px 0', justifyContent: 'center' }}>
        ...
        <div className="main-chart-box main-bar-box-final" style={{ flex: '2 1 700px', minWidth: 320, maxWidth: 900, width: '100%', background: '#fff', borderRadius: 22, boxShadow: '0 4px 16px rgba(160,132,232,0.10)', padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div className="visitor-count" style={{ fontSize: 22, fontWeight: 700, color: '#6a6cff', marginBottom: 12, marginTop: -4, textAlign: 'left', width: '100%' }}>{totalVisitors.toLocaleString()}</div>
